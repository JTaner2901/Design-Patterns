...
// ─────────────────────────────────────────────
// Ursprünglich von reactbits.dev/animations/cubes,
// hier neu implementiert und auf das Cyan/Schwarz-
// Theme der Seite umgestellt (Defaults unten).
// Gedacht als dezenter Hintergrund-Layer hinter einer
// Section, nicht als zentraler 50%-Block wie im Demo.
// ─────────────────────────────────────────────
...
const Cubes: React.FC<CubesProps> = ({
  ...
  cellGap,
  borderStyle = "1px solid rgba(0,210,255,0.25)",
  faceColor = "rgba(0,10,20,0.4)",
  shadow = false,
  autoAnimate = true,
  rippleOnClick = true,
  rippleColor = "rgba(0,210,255,0.8)",
  rippleSpeed = 2,
}) => {
    ...
    (rowCenter: number, colCenter: number) => {
      ...
      sceneRef.current
        ...
        .forEach((cube) => {
          ...
          const dx = colCenter - c;
          const dy = rowCenter - r;
          const dist = Math.hypot(dx, dy);
          if (dist <= radius && dist > 0.0001) {
            const pct = 1 - dist / radius;
            const angle = pct * maxAngle;
            // Normalisierte Richtung vom Würfel zum Cursor — der Würfel
            // "lehnt" sich in Richtung Maus, egal aus welcher Seite sie kommt.
            const nx = dx / dist;
            const ny = dy / dist;
            gsap.to(cube, {
              duration: enterDur,
  ...
  return (
    <div className="bg-cubes" style={wrapperStyle}>
      <div ref={sceneRef} className="bg-cubes--scene" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => (
